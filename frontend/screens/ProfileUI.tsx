...
import { getStoreName } from "@/utils/marketHelpers";

const MAX_WIDTH = 1024;
const TAB_BAR_OFFSET = 120;
const PACKAGE_NAME = "com."; // e.g., com.mysteryplay.app

const handleMyketIntent = (action: "comment" | "developer") => {
  // 'comment' opens the rating page directly
  // 'developer' opens your developer page
  const url = `myket://${action}?id=${PACKAGE_NAME}`;

  Linking.canOpenURL(url).then((supported) => {
    if (supported) {
      Linking.openURL(url);
    } else {
      // Fallback to web browser if Myket isn't installed
      const webUrl =
        action === "comment"
          ? `https://myket.ir/app/${PACKAGE_NAME}`
          : `https://myket.ir/developer/${PACKAGE_NAME}`; // Or your Dev ID
              ...
              <TouchableWithoutFeedback>
                <View style={styles.modalContent}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => handleMyketIntent("comment")}
                  >
                    <View style={styles.settingLabelContainer}>
                      <Ionicons
                        name="star-outline"
                      ...
                      <Text style={styles.settingText}>
                        {t("settings.rateUs", "Rate us on")} {getStoreName()}
                      </Text>
                    </View>
                    <Ionicons
