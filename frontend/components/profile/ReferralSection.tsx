  ...
  const handleShare = async () => {
    try {
      ...
      if (Platform.OS === "web" && !navigator.share) {
        ...
        Alert.alert(
          "Link Copied",
          "Share functionality is limited on this browser. Code copied to clipboard!",
        );
        return;
      }
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.title}>
            {t("profile.inviteFriends", "INVITE CREW")}
          </Text>
          <Text style={styles.subtitle}>
            {t("profile.inviteSubtitle", "Share code & level up")}
          </Text>
        </View>
...
const createStyles = (theme: any) =>
  StyleSheet.create({
    container: {
      ...
      ...Platform.select({
        web: {
          boxShadow: `0px 4px 15px ${theme.primary}40`, // 40 = hex opacity
        },
      }),
    },
    screw: {
