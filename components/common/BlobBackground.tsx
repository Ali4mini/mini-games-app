...
const OrganicBlob: React.FC<BlobProps> = ({ 
  ...
  useEffect(() => {
    // 1. Random "Floating" (Both X and Y to look like drifting clouds)
    translateY.value = withRepeat(
      withSequence(
        withTiming(-20, { duration: moveDuration / 2, easing: Easing.inOut(Easing.quad) }),
        withTiming(0, { duration: moveDuration / 2, easing: Easing.inOut(Easing.quad) })
      ),
      -1,
      true
    );
    ...
    translateX.value = withRepeat(
      withSequence(
        withTiming(15, { duration: moveDuration / 3, easing: Easing.inOut(Easing.quad) }),
        withTiming(0, { duration: moveDuration / 3, easing: Easing.inOut(Easing.quad) })
      ),
      -1,
      true
    );

    // 2. Slow Rotation for morphing effect
    ...
    opacity.value = withDelay(
      ...
      withRepeat(
        withSequence(
          withTiming(baseOpacity + 0.08, { duration: fadeDuration, easing: Easing.inOut(Easing.ease) }),
          withTiming(baseOpacity, { duration: fadeDuration, easing: Easing.inOut(Easing.ease) })
        ),
        -1,
        true
      )
      ...
      */}
      <View 
        style={[
          ...
          { backgroundColor: isDark ? 'rgba(0,0,0,0.02)' : 'rgba(255,255,255,0.4)' }
        ]} 
      />
